...
A dropped treatment indicator is different again, and is now a warning. The other two cases leave a treatment coefficient that means something narrower than the caller asked for; this one leaves no treatment coefficient at all, printed as an `NA` under a name that is still there. A treatment indicator can be collinear with the rest of a Lin design only where its arm is empty or where the covariates reproduce it exactly, so there is no reading of such a fit that recovers an effect. Reaching it takes a constant treatment column under the order-preserving drop rule, since the only column ahead of the treatment is the intercept; 2.0's norm-ranked pivot reached it on 6 of 30 rank-deficient fits of one published replication, which is the pairing that makes the warning worth having.
...
The consequence is that 2.0 silently changed which coefficient comes back `NA` on every rank-deficient fit, relative to both 1.0.6 and `lm()`. On a published replication whose subgroup models fit ten covariates in cells of 29 to 370 observations, 30 of 56 fits are rank deficient; `lm()` drops the treatment column in 0 of those 30, and 2.0 drops it in 6. Over 300 random rank-deficient designs, 2.0's drop set matched `lm()`'s on 57 of 300, and 2.0.1's matches on 300 of 300 with the surviving coefficients equal to `lm()`'s.

`lm_solver()` and the `HC2`, `HC3`, and `CR2` meat share the rule, so the variance is read off the same columns the coefficients were fitted at.

None of this makes an unidentified coefficient identified. Where a regressor lies in the span of the others no unique coefficient exists, the reported number still moves when the columns are reordered, and `lm()` reports such numbers with no indication that they are arbitrary. What changes is that the choice is now predictable and follows the order the model was written in.

## `try_cholesky = TRUE` could return a full set of coefficients at a rank-deficient design

`try_cholesky = TRUE` asks for the faster arithmetic, and it is meant to select the arithmetic rather than the answer. That path tests rank by reading the diagonal of the Cholesky factor of the normalised Gram matrix, where each entry stands for one column's residual after the earlier columns are projected out, and it falls back to the QR when an entry is small. A Gram matrix carries about half the digits of the residual its factor describes, so an exactly dependent column, whose residual share is `1e-16`, comes back with a diagonal entry of order `1e-8` to `1e-6` depending on how the cancellation falls, and the test was against `dqrdc2`'s own `1e-7`. Whether the fallback fired was therefore decided by rounding. On `y ~ a1 + a2 + a3 + b1 + b2 + b3` at 200 observations, with `a2` equal to `a1` plus 5% noise, `a3 = a1 - a2`, and `b1`, `b2`, and `b3` built the same way, the smallest entry lands at `2.4e-7`, no fallback fired, and the fit returned seven finite coefficients at a design of rank five, where `lm()` and the QR path both return two `NA`.

The threshold is now the Cholesky's own resolution rather than `dqrdc2`'s, so a design whose factor cannot separate the columns goes to the QR, which makes the rank decision at `1e-7` in full precision. No design above the threshold changes, since the fallback selects the path and not the answer. The near-dependencies that reach this code elsewhere are built as `a + eps * noise`, which does not cancel and so lands below `1e-7`, which is why the gap survived a test that walks a design from full rank to singular in twelve steps and compares both paths against `lm()` at each one.
...
The second stage regresses the outcome on the first-stage fitted values, and `lm_solver()`'s pivoted QR then ranked columns by norm rather than by position. On `y ~ x | z` with a flat first stage it dropped the intercept and returned the intercept's own value as the coefficient on `x`, which is the number a reader takes for the LATE. The drop set is now chosen before the fit: the intercept and the exogenous regressors are kept, since the first stage reproduces them exactly, and the endogenous regressors the instruments cannot reproduce come back as `NA`, in reverse formula order, as `stats::lm()` drops a collinear column. The surviving coefficients, standard errors, and degrees of freedom are the reduced model's to 2e-15.
...
The maintainer changes with this release, from Graeme Blair to Alexander Coppock. Graeme Blair remains an author. Macartan Humphreys, an author of the 1.x releases, has asked to be listed as a contributor instead, and his role in `Authors@R` moves from `aut` to `ctb`. Nothing else about the authorship changes.
...
**`tidy()`, `glance()`, and `augment()` return tibbles**, as broom's methods do; 1.x returned plain data frames. `$`, `[[`, and row indexing work as before. The one construction that changes is `tidy(fit)[, "estimate"]`, which now returns a one-column tibble rather than a vector; write `tidy(fit)$estimate`. tibble moves from Suggests to Imports.
...
The same count reached further than `nobs`. A cluster whose every row has weight 0 was still a cluster: `nclusters` counted it, the CR0 and stata degrees of freedom were one too many, the stata scale factor used it, and the F statistic's denominator degrees of freedom did. A zero-weight row left inside a cluster's CR2 block moved the weighted CR2 standard errors by about 8e-4 relative to the fit without it, where clubSandwich's CR2 does not move. A fixed-effect group whose every row has weight 0 was still a level, so the absorbed rank was one too many. `adj.r.squared` used the row count where `df.residual` used the observation count. `difference_in_means()` reported `nobs` as the row count under weights. 2.0 counts a zero-weight row as absent from every one of these: the fit with zero weights equals the fit on the rows with positive weight, field for field, for every estimator and every `se_type` (`test_equivalence.R`). Under two or more absorbed factors a zero-weight group's empty sum reached the eigendecomposition as `NaN`; 1.0.6 returned `NA` standard errors there, and 2.0 answers.
...
**An underidentified `iv_robust()` model returns estimates.** With fewer excluded instruments than endogenous regressors, 1.0.6 fits the second stage anyway, drops whichever regressor the instruments cannot reproduce, and reports the rest with standard errors. On `mpg ~ hp + cyl | am` it drops the intercept and returns coefficients on `hp` and `cyl`, under a warning that there are more regressors than instruments. When the count is sufficient but an instrument is a copy of an exogenous regressor, as in `y ~ en + x1 | x1 + dup` with `dup` equal to `x1`, it drops `en`, the endogenous regressor, and warns about nothing. **The direction is unknown, which is the problem: what comes back is not a 2SLS estimate of anything, and it looks like one.** 2.0 errors, comparing the rank of the first-stage fitted values with the rank of the regressors, which catches both cases and does not fire on regressors that are merely collinear among themselves. 2.0.1 returns `NA` for the unidentified regressor instead of erroring, choosing which regressor that is rather than leaving it to the pivot; see the 2.0.1 notes above.

**A clustered `iv_robust()` fit reported an overidentification test that ignores the clustering.** With `diagnostics = TRUE` and `clusters`, 1.0.6 returns Wooldridge's heteroskedasticity-robust score statistic, the same number the model gives unclustered. Where the instruments and the errors both vary at the cluster level, that variance is too small and the statistic too large: in 1,000 draws of 50 clusters of 10 with valid instruments, the test rejects at the 5% level 36.1% of the time, and the median statistic is 2.7 times the correct one. **The direction is toward rejection, so valid instruments are declared invalid.** 2.0 sums the score's variance within clusters, which rejects 4.0% of the time on the same draws, and returns `NA` with a warning when there are no more clusters than restrictions. Stata's `estat overid` computes nothing after `vce(cluster)`. A weighted over-identified fit, where 1.0.6 returned `NA` for this test without saying so, now gets the robust score test under any robust `se_type`, built on the weighted fit's sandwich variance. On `mtcars`, with 2SLS of `mpg` on `hp` and `gear` instrumented by `wt`, `am`, and `gear` and weighted by `drat / 5`, that gives 1.702. Stata's `forceweights` value of 1.845 is a different statistic rather than a competing answer: it is the frequency-weight score test, equal to the unweighted test on the data with each row repeated as often as its weight and rescaled to N rows, which is right only when a weight counts identical, independent copies of a row. Under `se_type = "classical"` with weights it gets Sargan's statistic on the weighted model, which matches Stata's `forceweights` value to its printed precision. With weights, every diagnostic is now the test on the model with each row multiplied by the square root of its weight. The first-stage F and Wu-Hausman tests are Wald tests under the fit's own `se_type`, so each is valid under exactly the conditions the matching standard errors are: a classical test when the weights are inverse error variances, a robust test under heteroskedasticity. The robust score test uses the score's HC0 or CR0 sandwich under every robust `se_type`, as Wooldridge (1995) and Stata define it; the HC1, HC2, HC3, CR2, and `"stata"` refinements correct a coefficient covariance and have no counterpart in a score test. Stata declines `estat endogenous` and `estat overid` after aweights unless forced, since it cannot tell precision weights from sampling weights, and computes the first-stage F regardless.
...
**Wu-Hausman endogeneity test wrong numerator df.** `wu_hausman_reg_ftest()` computed `numdf` as the number of columns in the first-stage residual matrix, which over-counted by one. `numdf` is now `lm_resids$rank - lm_noresids$rank`, the actual rank increase from adding the residuals.

The auxiliary regression was also being handed a first-stage residual for every regressor rather than for the endogenous ones only. An exogenous regressor appears in the instrument matrix, so its first-stage residual is zero up to rounding, and the correct count survived only because rank detection dropped that column of noise. It is not safe to rely on: both `stats::lm()` and a pivoted QR at `tol = 1e-7` keep such a column, and the scale-invariant rank detection described below does too, which adds a spurious degree of freedom to the numerator. `iv_robust()` now builds the auxiliary design from the endogenous residuals alone, so the test no longer depends on how rank is determined.

**Which regressors are endogenous is now decided by the column space, not by name.** The fix above still took the endogenous regressors to be the ones whose names do not appear in the instrument formula. An exogenous regressor that the instruments span without naming it, as in `y ~ en + x1 | v1 + v2 + v3` with `v3 = x1 + inst`, was therefore counted as endogenous: its first-stage residual is rounding error, the auxiliary regression carried that column, and the test returned 0.144 on 2 degrees of freedom where `y ~ en + x1 | inst + inst2 + x1`, the same model, returns 0.109 on 1. `AER::ivreg()` and estimatr 1.0.6 return 0.109 on 1 for both spellings, so this was a regression in the rewrite and never shipped. A regressor now counts as exogenous when the instruments reproduce it, its first-stage residual having norm at most `1e-7` of its own, and the weak-instrument F and the overidentification test are computed on a basis for the excluded instruments rather than on whichever columns happen to be named. All three diagnostics now depend only on the model, which `test_invariance.R` checks by writing the instruments three ways. Where the instrument formula names every exogenous regressor, no number moves beyond rounding. In the unnamed spelling the first-stage F now reports `en` alone, where `AER::ivreg()` also reports a row for `x1` with a statistic of 2.7e32.

### Multi-way fixed-effects demeaning stopped early on an outcome in small units

With two or more `fixed_effects` factors the demeaning iterates, and it stopped once a sweep changed nothing by more than `1e-8 * (1 + max|value|)`. The `1` made that test absolute whenever the values are small, and the outcome is demeaned on its own. An outcome measured in units of `1e-6` stopped after half the sweeps a unit-scale one takes. On a two-way design the coefficients were right, and the residuals came back wrong by `4.4e-4`, the fitted values by `3.2e-3`, and the standard errors by `7.1e-7`, relative to the same fit in unit scale; in units of `1e-9` the residuals were meaningless. The test is now relative to the values' own magnitude, with a floor for a column the fixed effects absorb entirely, and outcome scales from `1e-9` to `1e6` agree to `3e-14`. On the benchmark design the number of sweeps and the time are unchanged. estimatr 1.0.6 has a milder form of the same failure, with standard errors off by `1.4e-5` at `1e-9`. Found by `test_invariance.R`, which rescales the outcome of every estimator.
...
`lm_robust(y ~ x)` with `x` constant returned coefficients of order 1e11 instead of NA. The design matrix is exactly rank deficient, but Eigen's default `ColPivHouseholderQR` threshold is roughly `epsilon * ncol` relative to the largest pivot, tight enough that the collinear column survives as a pivot of order 1e-14. `stats::lm()` uses LINPACK `dqrdc2` with `tol = 1e-7`, and 2.0 sets the same threshold.

The threshold alone is not the whole of what `dqrdc2` does, and setting only the threshold introduced a second failure in the opposite direction, caught before release. Eigen compares every pivot against the largest pivot in the matrix, while `dqrdc2` compares each column's remaining norm against its own original norm. The second test is scale invariant and the first is not, so a single regressor measured in large units pushed the others below `1e-7` times the largest pivot and they were dropped as collinear on a design of full rank. Three independent normals and an intercept, with one column multiplied by `1e9`, lost three coefficients to `NA`; the effect begins at a condition number of about `1e7`, and it reached `lm_robust()` and `iv_robust()` alike, since `try_cholesky` defaults to `FALSE`. A polynomial regression discontinuity in an unscaled running variable is the ordinary case that hits it.

2.0 therefore normalizes the columns of the design to unit norm before the rank-revealing QR and undoes the scaling afterwards, at both the coefficient path and the `getMeatXtX()` path the hat values are read off. The criterion is then per column, which is what `dqrdc2` does and what the `1e-7` threshold was meant to reproduce. Exactly collinear columns are still dropped, and a full-rank design now returns the same coefficients whatever units its columns are measured in. `test_invariance.R` asserts that property directly, sweeping a column through twelve orders of magnitude and requiring every coefficient and standard error to come back unchanged.

**The same normalization fixed a third failure, on the `try_cholesky = TRUE` path, and this one is a difference from 1.0.6 rather than a pre-release catch.** `lm_robust(try_cholesky = TRUE)` solves through a Cholesky decomposition of `X'X` instead of the pivoted QR, and it was returning a coefficient for every column of a rank-deficient design, splitting arbitrarily between collinear ones, where the QR path and `stats::lm()` return `NA`. It warned about the resulting `NaN` standard error and never about the collinearity. The guard that was supposed to prevent it tested `llt.info()`, which cannot catch the case: Eigen's LLT reports success on a numerically singular Gram matrix. With the columns normalized, each diagonal entry of the Cholesky factor is the norm of that column's residual after the earlier columns are projected out, measured against its own original norm, which is `dqrdc2`'s criterion again; the path now falls back to the QR below the same `1e-7`. A design that is merely ill conditioned falls back too and pays the QR's cost rather than returning a bad answer. **`try_cholesky` is off by default, so a user reaches this only by asking for it, and estimatr 1.0.6 has the behaviour still.** The two paths now agree to `3e-17`, and the argument no longer changes the answer, only the speed.

A near-saturated design also produces observations that the fit reproduces exactly, whose leverage is 1. That case costs nothing in the answer: the residual is exactly 0, HC2's contribution is a 0/0 that resolves to 0, and the standard error comes back finite, though built from fewer rows than the fit used. The failure is leverage computed marginally *above* 1, which rounding produces on the same designs and which no projection diagonal can actually take. `1 - h` is then a small negative number, and the two estimators failed differently: HC2 divided by it and took the square root of a negative meat, so every standard error in the fit was `NaN` however small the offending term, while HC3 squared the denominator, which cancels the sign, and returned a finite number carrying a large spurious term without warning. The silent one was the worse failure.

Both failures are 1.0.6's as well as the rewrite's, confirmed by running it: on the probe design 1.0.6 returns `NaN` for HC2 and `39.5` for HC3 against a classical `1.42`, silently. 2.0 sets the denominator to 0 wherever `1 - h <= 0`, so both estimators drop those observations from the variance instead, and warns with a count of how many sat at or near leverage 1 and which `se_type` read it. The count is a tolerance, `h > 1 - sqrt(.Machine$double.eps)`, which is `sandwich::vcovHC()`'s criterion on the same quantity, rather than a test on the sign of `1 - h`. The reason is that on an exactly saturated design the standard error is the same whichever side of 1 the rounding puts the hat value, so a sign test would have left the warning to a single unit in the last place: the solver used here returns `1 + 2.2e-16` on such a design where `qr()` and `stats::hatvalues()` return exactly 1. A hat value is dimensionless and bounded by 1, so a fixed tolerance on it means the same thing in every design. `CR2` never forms `1 - h` and needs no guard: it eigendecomposes a per-cluster matrix and keeps only directions above `1e-12`, so a cluster that is fitted exactly already contributes 0. One consequence is worth naming: on a design with a singleton fixed-effect group, the absorbed fit and the same model with explicit dummies used to disagree, the first finite and the second `NaN`. They now agree, in the warning as well as in the number.
...
- **`iv_robust()` with multiple outcomes and `diagnostics = TRUE` died** in the score test with "dims do not match the length of object", in 1.0.6 as well. The diagnostic fields hold one statistic each, so the fit now warns that diagnostics are not available with multiple outcomes and skips them, as it does under `fixed_effects`.
- **`horvitz_thompson()` with a `cbind()` outcome answered for the first outcome alone**, silently: the matrix response was flattened and the first n values read as the outcome. 1.0.6 errored on the length mismatch. It now refuses and says to fit one model per outcome.
...
- **`lh_robust()` under CR2 gave a combination of coefficients the wrong degrees of freedom.** 1.0.6 refused CR2 in `lh_robust()`. The rewrite allowed it, and gave any hypothesis that was not a single coefficient the smallest of the coefficients' Satterthwaite degrees of freedom, on the stated grounds that this was conservative. It is not a bound: on the test data it was 16.10 where the combination's own is 15.75, which narrows the interval. Each hypothesis now gets its own Satterthwaite degrees of freedom, computed from the same CR2 components as the coefficients', and matches `clubSandwich::linear_contrast()` unweighted, weighted, and with absorbed fixed effects. The joint test's denominator takes the smallest of them.
- **Horvitz-Thompson probabilities that cannot be probabilities.** `horvitz_thompson()` accepted probabilities outside [0, 1] and returned an ordinary-looking estimate and standard error from them, and a probability of 0 for a condition a unit was observed in returned `NaN` or `Inf` without a word. Both are now refused, the second naming the condition and how many units it affects; the check runs after every `condition_prs` format has been turned into per-unit probabilities, so it covers them all.
- **Infinite weights, and weights that are all zero.** Both returned every coefficient as `NA` under a warning calling the regressors collinear, in 1.0.6 as well, where `lm()` refuses an infinite weight. Both are now refused with a message saying which. A missing weight is still a dropped row.
