...
test_that("We match Joel's estimator", {
  ...
  dmat.CR<-round(rbind(cbind(d.00, -d.00)
                       , cbind(-d.00, d.00)
  ), 10)
  ...
  expect_equal(
    tidy(ht_decl_o)[, c("est", "se")],
    tidy(ht_prob_o)[, c("est", "se")]
  )
  expect_equivalent(
    tidy(ht_decl_o)[, c("est", "se")],
    c(ht_est, se_ht)
