...
#
#   gto <- modelsummary(list(model1, model2), gof_omit = c("N|[sS]tatistic|p.value|p{1}"))
#
#   expect_equal(nrow(gto), 7L)
#
#   # difference_in_means
#   model1 <- difference_in_means(mpg ~ am, mtcars)
#   model2 <- difference_in_means(mpg ~ am, mtcars, blocks = vs)
#
#   gto <- modelsummary(list(model1, model2))
#
#   expect_equal(nrow(gto), 8L)
#
#     # horvitz_thompson
