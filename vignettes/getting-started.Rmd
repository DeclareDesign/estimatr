...
# Ten blocks of ten units, defined by decile of the covariate
dat$block <- cut(dat$x, breaks = quantile(dat$x, seq(0, 1, 0.1)),
                 labels = FALSE, include.lowest = TRUE)
```

The three assignments come from randomizr, whose declarations `horvitz_thompson()` reads later on.

```{r, eval = has_randomizr}
library(randomizr)

dat$z <- complete_ra(N, m = N / 2)
dat$z_cl <- cluster_ra(clusters = dat$cl, m = 10)
dat$z_bl <- block_ra(blocks = dat$block, prob = 0.5)
```

```{r, echo = FALSE, eval = !has_randomizr}
# Base R equivalents, so the document still builds without randomizr installed.
dat$z <- 0
dat$z[sample(N, N / 2)] <- 1
dat$z_cl <- as.integer(dat$cl %in% sample(unique(dat$cl), 10))
dat$z_bl <- ave(dat$z, dat$block, FUN = function(g) sample(rep(0:1, each = 5)))
```

Each assignment reveals one of the two potential outcomes.
...
`fixed_effects` takes a right-sided formula and absorbs those groups rather than adding them as dummy columns. The estimates and standard errors are identical to writing the dummies out, and no dummy matrix is ever built, at any number of factors, which is what makes wide fixed effects cheap.
...
Design-based work means fitting the same model many times over, which is what estimatr is built to do quickly. A sampling distribution of the Lin estimator under this design is a loop:

```{r}
sim_once <- function() {
  d <- dat
  d$z <- sample(d$z)
  d$y <- ifelse(d$z == 1, d$y1, d$y0)
  lm_lin(y ~ z, covariates = ~ x, data = d)$coefficients[["z"]]
}

set.seed(1)
sims <- replicate(500, sim_once())
c(mean = mean(sims), sd = sd(sims), truth = 0.35)
```

Swapping `lm_robust()` for `lm_lin()` in that loop is how you compare two estimators on one design.
