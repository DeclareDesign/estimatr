...
Usage largely mimics `lm()`, although it defaults to using Eicker-Huber-White robust standard errors, specifically 'HC2' standard errors. More about the exact specifications used can be found in the [technical notes](#lm_robust-notes).
...
While estimating differences in means may seem straightforward, we provide a function that appropriately adjusts estimates for experimental design, whether it be blocking, clustering, matched pairs or a more complicated design. Usage is similar to usage in regression functions and here are a few short examples; more can be seen on the function reference page [`difference_in_means`](#difference_in_means).

Technical details are available [here](#lm_robust-notes).
