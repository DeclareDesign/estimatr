...
One hundred units and a pre-treatment covariate, with the same experiment run three ways: complete random assignment of half the sample, cluster assignment of ten of twenty clusters, and block assignment of five units within each of ten blocks. The three assignments come from randomizr, whose declarations `horvitz_thompson()` reads later on.
...
dat <- tibble(
  ...
  y1 = y0 + 0.35, # treatment potential outcome, a constant effect
  w = runif(N, 0.5, 1.5), # sampling weights, used further down
  inst = x + rnorm(N, sd = 0.3), # an instrument that shifts x, used further down
  block = cut(x, breaks = quantile(x, seq(0, 1, 0.1)),
              labels = FALSE, include.lowest = TRUE),
  z = complete_ra(N, m = N / 2),
  z_cl = cluster_ra(clusters = cl, m = 10),
  z_bl = block_ra(blocks = block, prob = 0.5),
  y = ifelse(z == 1, y1, y0),
  y_cl = ifelse(z_cl == 1, y1, y0),
  y_bl = ifelse(z_bl == 1, y1, y0)
)

print(dat, n = 5, width = Inf)
```

The true average treatment effect is 0.35 by construction, so you can see how each estimator does, and also how far a single draw can land from the truth: the clustered and blocked assignments below both happen to fall a couple of standard errors low. That spread is what the simulation at the end of this vignette measures.
...
Without clusters, `se_type` takes `"classical"`, `"HC0"`, `"HC1"`, `"HC2"`, and `"HC3"`; with them, `"CR0"` and `"CR2"`. `"stata"` is accepted in both cases and reproduces what Stata's `robust` or `cluster` option gives, which is HC1 without clusters and CR0 with Stata's two small-sample corrections with them. Reproducing Stata is a matter of asking for it.
...
sim_once <- function() {
  z_sim <- sample(dat$z)
  d <- tibble(x = dat$x, z = z_sim, y = ifelse(z_sim == 1, dat$y1, dat$y0))
