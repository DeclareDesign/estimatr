...
This document exposes the properties of different variance estimators using [`DeclareDesign`](http://declaredesign.org/R/DeclareDesign/) and [`estimatr`](http://declaredesign.org/R/estimatr/). More details about the variance estimators with references can be found in the [mathematical notes](http://declaredesign.org/R/estimatr/articles/mathematical-notes.html).
...
Under simple conditions with homoskedasticity (i.e., all errors are drawn from a distribution with the same variance), the classical estimator of the variance of OLS should be unbiased. In this section I demonstrate this to be true using [`DeclareDesign`](http://declaredesign.org/R/DeclareDesign/) and [`estimatr`](http://declaredesign.org/R/estimatr/).
...
Now, using [`DeclareDesign`](http://declaredesign.org/R/DeclareDesign/), let's specify the rest of the data generating process (DGP). Let's set $\beta = [0, 1]^\top$, so that the true DGP is $\mathbf{y} = \mathbf{x_1} + \epsilon$.
...
where the residuals $\mathbf{e} = \mathbf{y} - \mathbf{X}\widehat{\beta}$, $N$ is the number of observations, and $K$ is the number of regressors---two in our case. We can easily get this estimate of the variance by squaring the standard error we get out from `lm_robust` in [`estimatr`](http://declaredesign.org/R/estimatr/). Let's tell [`DeclareDesign`](http://declaredesign.org/R/DeclareDesign/) to use that estimator and get the coefficient on the $\mathbf{x}_1$ variable.
...
where $\sigma^2_i \neq \sigma^2_j$ for some units $i$ and $j$. If the variance of the errors is not independent of the regressors, the "classical" variance will be biased and inconsistent. Meanwhile, heteroskedastic-consistent variance estimators, such as the HC2 estimator, are consistent and normally less biased than the "classical" estimator. Let's demonstrate this using [`DeclareDesign`](http://declaredesign.org/R/DeclareDesign/). First, let's specify the variance of the errors to be strongly correlated with $x$.
