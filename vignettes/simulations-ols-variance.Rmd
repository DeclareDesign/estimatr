...
This document exposes the properties of different variance estimators using [`DeclareDesign`](declaredesign.org) and [`estimatr`](estimatr.declaredesign.org). More details about the variance estimators with references can be found in the [technical notes](http://estimatr.declaredesign.org/articles/technical-notes.html).
...
where the residuals $\mathbf{e} = \mathbf{y} - \mathbf{X}\widehat{\beta}$, $N$ is the number of observations, and $K$ is the number of regressors---two in our case. We can easily get this estimate of the variance by squaring the standard error we get out from `lm_robust` in [`estimatr`](estimatr.declaredesign.org). Let's tell [`DeclareDesign`](declaredesign.org) to use that estimator and get the coefficient on the $\mathbf{x}_1$ variable.
...
sumvar <- summary(dig$simulations$se^2)
ggplot(dig$simulations, aes(x = se^2)) +
  geom_density() +
  geom_vline(xintercept = true_varb, size = 1, color = "red") +
  geom_text(aes(x = true_varb, y = 0, label = "True Variance"), color = "red", vjust = -0.5, hjust = -0.1) +
...
ggplot(plotdf, aes(x = N, y = `Bias of Estimated Variance`, color = estimator_label)) +
  geom_point() +
  geom_hline(yintercept = 0, type = 2, color = "grey") +
