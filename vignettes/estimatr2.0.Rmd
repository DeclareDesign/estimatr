...
**The second layer asks whether the answer is right, which the first cannot.** Anything estimatr inherited from 1.0.6, error included, passes a comparison against 1.0.6 in silence. So 305 further assertions compare against implementations built independently of this one:
...
The test suite is 1,729 assertions with none failing, `R CMD check --as-cran` is clean, and the numerical comparisons against 1.0.6 run across 50 seeds per design type for every Horvitz-Thompson design family.
