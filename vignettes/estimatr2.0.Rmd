...
**Not to make the package smaller.** Counted the same way on both branches, `R/` and `src/` together are 6,866 lines in 1.0.6 and 6,334 in 2.0, a reduction of 8 percent, and with blank lines and comments stripped the two are within half a percent of each other, 4,072 against 4,050. The rewrite is the same amount of code doing different work.

What changed is where the complexity sits, and that is the interesting part. Roxygen went from 1,597 lines to 773, because the S3 surface is generated rather than written out per class. C++ grew, from 509 lines to 692. The savings were real and they were spent: on the Pashley and Miratrix blocked-variance estimator, which 1.x does not have at all, on a wider `predict()`, and on the error messages that this release adds wherever 1.x returned a silent number.
