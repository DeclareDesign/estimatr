...
estimatr 2.0.0 was written with AI. `vignette("mathematical-notes")` is where that is set out, alongside the guarantee it buys you: every estimator here is defined in mathematics and then validated to machine precision against that definition, measured when the vignette is built.

New to the package rather than porting a script? `vignette("getting-started")` walks through the six estimators, and `vignette("mathematical-notes")` gives the definition of each with its citation.

This version is checked twice over besides: against a recording of an installed estimatr 1.0.6, which shows the rewrite moved no answer, and against implementations that share no lineage with it (`sandwich`, `clubSandwich`, `ivreg`, Stata, `fixest`, `plm`, `blkvar`), which shows the answers are right. [How this was checked] sets out both layers.

## The short version

estimatr 2.0 is a ground-up rewrite aimed at the DeclareDesign workflow: fit the same model thousands of times, as fast as possible, and get the same number every time. Almost everything you already wrote against 1.x runs unchanged and returns identical estimates, identical standard errors, identical confidence intervals, and identical objects, only faster. That claim is pinned by 2,236 test assertions, 695 of which compare against answers recorded from an installed estimatr 1.0.6, coefficient by coefficient and standard error by standard error, and a further 268 of which compare against implementations that share no lineage with estimatr at all. See [How this was checked].
...
The gains concentrate where the algorithms changed. Ordinary `lm_robust()` is about two and a half times as fast. Absorbed fixed effects are about 470x faster at 500 blocks and about 10,000x faster at 2,000, returning bit-identical numbers. Horvitz-Thompson under complete randomization is about 1,000x faster at N = 3,000, because the N-by-N joint inclusion probability matrix is never built.
...
**2.0 is not a smaller package, and an earlier version of this section said it was.** Counted the same way on both branches, `R/` and `src/` together are 6,866 lines in 1.0.6 and 6,334 in 2.0, a reduction of 8 percent. Strip blank lines and comments and the two are within half a percent of each other, 4,072 against 4,050. The claim that 2.0 was half the size came from comparing 1.x's total lines against 2.0's code-only lines, which is two measurements rather than one comparison, and it does not survive counting both sides alike.

What did happen is that the complexity moved, and where it moved is the interesting part. Roxygen went from 1,597 lines to 773, because the S3 surface is generated rather than written out per class. C++ grew, from 509 lines to 692. The savings were real and they were spent: on the Pashley and Miratrix blocked-variance estimator, which 1.x does not have at all, on a wider `predict()`, and on the error messages that this release adds wherever 1.x returned a silent number.

The Horvitz-Thompson variance was the largest saving and the one that looked least likely. 1.x builds the full N-by-N matrix of joint inclusion probabilities, with 538 lines of helpers to construct it for each design family. Young's inequality bound for any complete-randomization design reduces to six scalar sufficient statistics per block, so the matrix never has to exist. Designs that cannot be described parametrically are a permutation matrix, and `tcrossprod()` handles those in one line. Counting code lines on both sides alike, the estimator and its helper go from 746 lines to 339, and it is faster by a factor that grows with N.
...
**The second layer asks whether the answer is right, which the first cannot.** Anything estimatr inherited from 1.0.6, error included, passes a comparison against 1.0.6 in silence. So 268 further assertions compare against implementations built independently of this one:
...
The test suite is 2,236 assertions with none failing, `R CMD check --as-cran` gives 0 errors, 0 warnings and 1 NOTE for the maintainer change, and the numerical comparisons against 1.0.6 run across 50 seeds per design type for every Horvitz-Thompson design family.
