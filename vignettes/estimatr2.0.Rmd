...
Three things break, and each one is a deliberate trade. `horvitz_thompson()` takes one probability argument instead of five. Two or more fixed-effect factors no longer accept HC2 or HC3, and no fixed-effects model accepts CR2. Two stargazer helpers are gone. Exactly one change moves a number without saying so, and it is named in the porting section.
...
### 2. `fixed_effects` refuses CR2, and refuses HC2 and HC3 with two or more factors
...
**Two or more factors keep the restriction**, because the analogous sum is wrong in the third decimal place rather than by rounding. Those requests error and name the dummy formulation:

```{r, error = TRUE}
dat$bl2 <- rep(1:20, times = 50)
lm_robust(y ~ z + x, data = dat, fixed_effects = ~ bl + bl2, se_type = "HC2")
```

**`CR2` is refused with any `fixed_effects`**, one factor or several, because its adjustment is built from cluster-level blocks of the hat matrix rather than from `h_ii`, and the identity above says nothing about those.
...
**The upside.** You can absorb a very large number of fixed effects, which was previously impractical: 1.x defaults to HC2 with fixed effects and computes it from the full dummy hat matrix, so 40,000 observations across 2,000 blocks takes it 41 seconds. 2.0 returns the same number in 18 milliseconds. And where a standard error is genuinely refused, the error message names the escape hatch, so nothing is unavailable:

```r
# exactly equal to the fixed_effects + HC2 result, at the cost of the dummies
lm_robust(y ~ z + x + factor(bl) + factor(bl2), data = dat, se_type = "HC2")
```
...
| `fixed_effects` with `se_type = "CR2"`, or with `"HC2"`/`"HC3"` and two or more factors | `"CR0"` or `"stata"`, or write the dummies with `factor()` | absorbed fixed effects |
...
All of those stop with an error. The first two name their replacement; the third fails because the function does not exist; the fourth says that 1.x accepted a bare vector and that this one does not. Enforcing the formula is the resolution of issue #304.

**One change does not announce itself, and it is the only one.** `lm_robust(y ~ z, fixed_effects = ~ block, clusters = cl)` with no `se_type` named gets CR2 from 1.x and CR0 from 2.0. The point estimates are identical. CR0 typically runs a couple of percent below CR2, in the usual direction. Unclustered fixed effects are not affected: the default there is HC2 in both versions and the numbers are bit-identical. If a clustered fixed-effects result turns on which adjustment you used, name the `se_type` explicitly rather than taking the default.
