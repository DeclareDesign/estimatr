...
There are exactly two places where the numbers differ on purpose. Both involve `fixed_effects`, and both are described below.
...
### 2. `fixed_effects` changes which standard error you get by default

**One fixed-effect factor and no clusters is unaffected.** HC2 and HC3 are exact, the default is HC2, and both are what 1.x returns for the same call. This is the common case and it ports with no change to the numbers at all.

```{r}
fit_fe <- lm_robust(y ~ z + x, data = dat, fixed_effects = ~ bl)
fit_fe$se_type
```

The reason it works is an identity. HC2 and HC3 need the leverage values of the full design matrix, meaning the matrix with every fixed-effect dummy in it, and absorbing fixed effects by demeaning is exactly the decision not to build that matrix. But the projection onto the full design splits exactly into the projection onto the dummies and the projection onto the demeaned covariates:

```
P_[X | D] = P_D + P_{M_D X}
```

so `h_ii` is the demeaned-X hat value plus `diag(P_D)`. With a single factor `P_D` is diagonal and that second term is simply `w_i / sum(w over i's group)`.

**This holds for any number of factors.** With several, `P_D` is no longer diagonal, but writing the FE design with its widest factor in full dummies and the rest contrast-coded leaves the leading block of `D'WD` diagonal, so `diag(P_D)` needs only a factorisation of size `sum_{k>1}(g_k - 1)`: for a panel of many units over a few periods, a handful of columns. The dummy matrix is never built. Verified to machine precision against writing the dummies out, weighted and unweighted, balanced and unbalanced, from one factor to five.

What does *not* work is the tempting shortcut of adding up each factor's own within-group share. That drops the cross terms and is wrong in the third decimal place rather than by rounding; an earlier draft of 2.0 tested that version, found it wrong, and concluded there was no identity beyond one factor.

**CR2 is the exception.** Its adjustment is built from cluster-level *blocks* of the hat matrix rather than from `h_ii`, and blocks do not decompose the way the diagonal does. There 2.0 does what 1.x did: it writes the dummies out and reads the hat values off the full design. The answer is exact and equal to 1.0.6.

```{r}
dat$bl2 <- rep(1:20, times = 50)
lm_robust(y ~ z + x, data = dat, fixed_effects = ~ bl + bl2, se_type = "HC2")$std.error
lm_robust(y ~ z + x, data = dat, fixed_effects = ~ bl,
          clusters = cl, se_type = "CR2")$std.error
```

**What that costs is why one default does not do it.** Expanding the dummies is roughly cubic in the number of levels, and that cost is the whole reason to absorb fixed effects rather than expand them. Only CR2 still needs the expansion, so it is the only default that moves:

```{r}
fit <- lm_robust(y ~ z + x, data = dat, fixed_effects = ~ bl + bl2)
fit$se_type
```

Unclustered fixed effects keep the `"HC2"` default at any number of factors, exactly as 1.x did. Any `fixed_effects` with `clusters` defaults to `"CR0"` where 1.x gave `"CR2"`, and warns once per session rather than once per call, since absorbed fixed effects are usually fitted in a loop. Writing `se_type = "CR0"` accepts the new default and removes the warning; writing `se_type = "CR2"` gets the 1.x number back, also without warning.

**The upside.** You can absorb a very large number of fixed effects and get a standard error quickly, which was previously impractical. 1.x computes HC2 from the full dummy hat matrix, so 40,000 observations across 2,000 blocks takes it 41 seconds; 2.0 returns the same number in 18 milliseconds. Two-way is where it tells most: 50,000 observations across 1,000 x 30 groups took 13.8 seconds and 970 MB in 1.x, and takes 36 milliseconds and 174 MB here. The memory that made wide fixed effects impractical is simply never allocated.
...
HAC (Newey-West) and CR3 standard errors are still not implemented.
...
The first row stops with an error naming its replacement. The third fails because the function does not exist. The fourth still runs, and still gives the 1.x answer, but warns: enforcing the formula is the resolution of issue #304, and a warning enforces it without breaking a working script.
...
Fixed effects were the second. Absorbing by alternating projections in C++ is the `feols` algorithm, and it makes the number of fixed effects almost free. The one-way leverage identity then buys back HC2 and HC3 at the cost of a single vector, which is why absorption no longer costs a standard error type in the common case.
...
estimatr 2.0.0 was written by Alexander Coppock working with Claude (Anthropic), across design, implementation, tests, benchmarks, and documentation. The disclosure belongs next to the evidence, because the evidence is the part that should decide whether you install it.
...
**The second layer asks whether the answer is right, which the first cannot.** Anything estimatr inherited from 1.0.6, error included, passes a comparison against 1.0.6 in silence. So 298 further assertions compare against implementations built independently of this one:
...
estimatr matches `sandwich` and `clubSandwich` to machine precision everywhere the two overlap, weighted included, with the CR2 Satterthwaite degrees of freedom exact.
...
**One divergence from `sandwich` is deliberate, and it is the one to know about if you compare the two.** For `iv_robust()` with HC2 or HC3, estimatr computes leverage as `h = xhat'(Xhat'Xhat)^-1 xhat`, from the second stage alone; `sandwich` uses `h = xhat'(Xhat'Xhat)^-1 x`. Both sum to K, and the 2SLS hat matrix is idempotent but not symmetric, so the exact homoskedastic expectation is `1 - 2h + [HH']_ii` and neither convention computes it. The HC2 unbiasedness argument does not extend uniquely to 2SLS, so this is a choice rather than a bug on either side. What settles it here is boundedness: estimatr's is the diagonal of an orthogonal projection and so lies in [0, 1], which means HC2 is always defined, while `sandwich`'s is unbounded. Across 3,000 simulated weak-first-stage designs it exceeded 1 in 10.8% of them, reaching 309, and `sandwich` returns `NaN` there with a warning of its own. Where both are defined the two are interchangeable, with identical interval widths and identical coverage. estimatr's convention is also bit-identical to 1.0.6, so this is not something 2.0 changed.
...
The test suite is 1,722 assertions with none failing, `R CMD check --as-cran` is clean, and the numerical comparisons against 1.0.6 run across 50 seeds per design type for every Horvitz-Thompson design family.
