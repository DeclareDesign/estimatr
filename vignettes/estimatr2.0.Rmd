...
No call is slower, and the gains concentrate where the algorithms changed. See [Speed].

estimatr 2.0.0 was written with AI, and `vignette("mathematical-notes")` sets that out alongside the guarantee it buys you: every estimator is defined in mathematics and then validated to machine precision against that definition, measured when the vignette is built. The release is checked twice over besides, against a recording of an installed estimatr 1.0.6, which shows the rewrite moved no answer, and against implementations that share no lineage with it (`sandwich`, `clubSandwich`, `ivreg`, Stata, `fixest`, `plm`, `blkvar`), which shows the answers are right. [How this was checked] sets out both layers.

If you are new to the package rather than porting a script, `vignette("getting-started")` walks through the six estimators and `vignette("mathematical-notes")` gives the definition of each with its citation.
...
The short version states the claim. Here is the evidence behind it, which is the part that should actually decide whether you install this.
...
**The second layer asks whether the answer is right, which the first cannot.** Anything estimatr inherited from 1.0.6, error included, passes a comparison against 1.0.6 in silence. So 770 further assertions compare against implementations built independently of this one:
...
The test suite is 5,604 assertions with none failing, `R CMD check --as-cran` gives 0 errors, 0 warnings and 1 NOTE for the maintainer change, and the numerical comparisons against 1.0.6 run across 50 seeds per design type for every Horvitz-Thompson design family.
...
**A clustered `iv_robust()` fit reported an overidentification test that ignored the clustering.** With `diagnostics = TRUE` and `clusters`, 1.x returned the heteroskedasticity-robust score statistic, the number the same model gives unclustered. Where the instruments and the errors both vary by cluster, that statistic is too large: in 1,000 draws of 50 clusters of 10 with valid instruments, it rejected at the 5% level 36.1% of the time. The direction is toward rejection, so valid instruments were declared invalid. 2.0 sums the score's variance within clusters, which rejects 4.0% of the time on the same draws.

**A weighted `iv_robust()` fit computed the classical Wu-Hausman test, and returned `NA` for the overidentification test without saying why.** Under weighting, the classical forms of both statistics are not known to keep their reference distributions, which is why Stata declines both after aweights unless forced. 2.0 returns `NA` for both under `se_type = "classical"` with weights, and warns. Under a robust `se_type` it computes both from the weighted fit's sandwich variance.
