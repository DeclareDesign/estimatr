...
estimatr 2.0.0 was written with AI. `vignette("estimatr_guarantees")` is where that is set out, alongside the guarantee it buys you: every estimator here is validated to machine precision against the definition it implements, measured when that vignette is built.

New to the package rather than porting a script? `vignette("getting-started")` walks through the five estimators, and `vignette("mathematical-notes")` gives the definition of each with its citation.
