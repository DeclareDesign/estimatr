...
```
```{r echo=FALSE}
# knitr::kable(lm_robust(y ~ z + x, data = dat))
```
...
```
```{r echo=FALSE}
#knitr::kable(lm_robust(y_clust ~ z_clust + x, data = dat, clusters = clust))
```
...
```
```{r echo=FALSE}
#knitr::kable(lm_robust(y_clust ~ z_clust + x,
#                       data = dat,
#                       ckusters = clust,
#                       se_type = "stata"))
```
...
```
```{r echo=FALSE}
# knitr::kable(lm_lin(y ~ z, covariates = ~ x, data = dat))
```
...
```
```{r echo=FALSE}
#knitr::kable(difference_in_means(y ~ z, data = dat))
#knitr::kable(difference_in_means(y_clust ~ z_clust, data = dat, clusters = clust))
```
...
```
```{r echo=FALSE}
#knitr::kable(horvitz_thompson(y_clust ~ z_clust, 
#                 data = dat, 
#                 declaration = crs_clust_decl))
```
...
```
```{r echo=FALSE}
#knitr::kable(horvitz_thompson(y ~ z, 
#                 data = dat, 
#                 condition_pr_mat = arb_pr_mat))
```
