...
\[
\hat{\V}[\hat{\beta}]_{BM} =  \XtXinv \sum^S_{s=1} \left[\X^\top_s (I_{N_s} - \P_{ss})^{-\frac{1}{2}} \e_s\e^\top_s (I_{N_s} - \P_{ss})^{-\frac{1}{2}} \X_s \right] \XtXinv
\]
...
Also, $\P = \X\XtXinv\X^\top$ and $(I - \P)_s$ is the $N_s$ columns that correspond to cluster $s$.

We define $\G$ as an $N\times S$ matrix with each column as

\[
\G_s = (I-\P)_s (I_{N_s} - \P_{ss})^{-\frac{1}{2}} \X_s \XtXinv z_{K, k}
\]
...
\]
where $\lambda_i$ are the eigenvalues of $\G^\top\G$.
