...
vignette: |
  %\VignetteIndexEntry{Estimatr - Fast estimators for social scientists} %\VignetteEngine{knitr::rmarkdown} \usepackage[utf8]{inputenc}
---
...
dat <- fabricatr::fabricate(
  ...
  clust = sample(letters[1:5], size = N, replace = T),
...
Usage largely limics `lm()`, although it defaults to using Eicker-Huber-White robust standard errors, specifically 'HC2' standard errors:
...
```{r}
lm_robust(y_clust ~ z_clust + x, data = dat, clusters = clust)
```
...
```{r}
lm_robust(y_clust ~ z_clust + x, data = dat, clusters = clust, se_type = 'stata')
```
...
# Clustered version
difference_in_means(y_clust ~ z_clust, data = dat, clusters = clust)
```
...
crs_decl <- randomizr::declare_ra(N = nrow(dat), prob = 0.5, simple = FALSE)
horvitz_thompson(y ~ z, data = dat, declaration = crs_decl)
...
horvitz_thompson(y_clust ~ z_clust, data = dat, declaration = crs_clust_decl)
```
...
permutes <- matrix(rbinom(nrow(dat)*500, size = 1, prob = 0.5), nrow = nrow(dat))
dim(permutes)
permutes[1:5, 1:2]
...
```{r hide-technical-appendix, eval = F, echo = F}
